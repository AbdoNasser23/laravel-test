...
@section('content')
    ...
    @if( in_array($checkRole,['admin','viewer']))
...
@endif
    ...
    @if($post->published === 1 || $post->user_id === Auth::id() || in_array($checkRole,['admin']))
        <div
            ...
            <div class="flex items-center gap-3">
                @if((in_array($checkRole,['admin']) && $post->user_id === Auth::id()) || $post->user_id === Auth::id())
                <a href="{{ route('posts.edit', $post->id) }}" class="text-blue-500 hover:text-blue-700 font-medium">
                    Edit
                </a>
                @endif
                @if(in_array($checkRole,['admin']) || $post->user_id === Auth::id())
